...
--  Nguồn: TOOL PAK DX/REPACK/1/1.lua
-- ============================================================
    ...
    local _ao_ok, _ao_err = pcall(function()
        ...
        local function applySkinToWeaponRef(CurWeapon)
            ...
            if ok then
                ...
                DX_Log(string.format("[SKIN_LOCK] Súng [%s] -> Đã áp duy nhất 1 Skin ID [%s] (Không đè Mesh)", tostring(curWeaponResID), tostring(desiredSkin)))
            end
            return ok
        end
        ...
        -- Fix startMatchWatcher: Chạy 1 lần duy nhất khi vào trận để tránh vòng lặp đè skin
