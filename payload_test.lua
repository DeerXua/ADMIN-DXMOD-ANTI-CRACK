...
-- ESP HIỂM HỌA (Nút bình thường)
        ...
        AddSlider(StackSkin, "SkinSuit", "   Mẫu Trang Phục (1-50)", 1, 50, "ModMenu_ModSkin")
        AddSlider(StackSkin, "SkinBag", "   Mẫu Balo Cấp 3 (1-15)", 1, 15, "ModMenu_ModSkin")
        AddSlider(StackSkin, "SkinHelmet", "   Mẫu Mũ Cấp 3 (1-15)", 1, 15, "ModMenu_ModSkin")
        AddSlider(StackSkin, "SkinM416", "   Skin M416 (1-15)", 1, 15, "ModMenu_ModSkin")
        AddSlider(StackSkin, "SkinAKM", "   Skin AKM (1-15)", 1, 15, "ModMenu_ModSkin")
        AddSlider(StackSkin, "SkinSCAR", "   Skin SCAR-L (1-15)", 1, 15, "ModMenu_ModSkin")
        AddSlider(StackSkin, "SkinM762", "   Skin M762 (1-15)", 1, 15, "ModMenu_ModSkin")
        AddSlider(StackSkin, "SkinAUG", "   Skin AUG (1-15)", 1, 15, "ModMenu_ModSkin")
        AddSlider(StackSkin, "SkinUMP", "   Skin UMP45 (1-15)", 1, 15, "ModMenu_ModSkin")
        AddSlider(StackSkin, "SkinUZI", "   Skin UZI (1-15)", 1, 15, "ModMenu_ModSkin")
        AddSlider(StackSkin, "SkinGroza", "   Skin Groza (1-15)", 1, 15, "ModMenu_ModSkin")
        AddSlider(StackSkin, "SkinDBS", "   Skin DBS (1-15)", 1, 15, "ModMenu_ModSkin")
        AddSlider(StackSkin, "SkinS12K", "   Skin S12K (1-15)", 1, 15, "ModMenu_ModSkin")
        AddSlider(StackSkin, "SkinDacia", "   Skin Xe Dacia (1-15)", 1, 15, "ModMenu_ModSkin")
        AddSlider(StackSkin, "SkinUAZ", "   Skin Xe Jeep UAZ (1-15)", 1, 15, "ModMenu_ModSkin")
        AddSlider(StackSkin, "SkinCoupe", "   Skin Xe Coupe RB (1-15)", 1, 15, "ModMenu_ModSkin")
        AddSlider(StackSkin, "SkinBuggy", "   Skin Xe Buggy (1-15)", 1, 15, "ModMenu_ModSkin")
        AddSlider(StackSkin, "SkinMirado", "   Skin Xe Mirado (1-15)", 1, 15, "ModMenu_ModSkin")

        table.insert(StackSkin, {
            Key = "ModMenu_SkinUnlockAll",
            UI = AliasMap.Switcher,
            Text = "▶ BẬT/TẮT UNLOCK ALL SKIN (TRẬN)",
            GetFunc = function() return _G.HK_Settings.SkinUnlockAll == 1 or _G.LexusConfig.SkinUnlockAll == true end,
            SetFunc = function(_, v)
                local val = v and 1 or 0
                _G.HK_Settings.SkinUnlockAll = val
        ...
        SettingPageDefine.ModMenu = {
            ...
            Category = {
                ...
                { Key = "ModMenu_CatSkin", loc = "MOD SKIN", text = "MOD SKIN", Text = "MOD SKIN", title = "MOD SKIN", Title = "MOD SKIN", Stack = StackSkin },
...
-- =========================== PHẦN 31: INTEGRATE UNIVERSAL SKIN CHANGER SYSTEM ===========================
...
local function X3UniversalUnlock()
    ...
    for _, h in ipairs(hooks) do
        ...
        if type(mod) == "table" then
            for _, fn in ipairs(h.fns) do
                if type(mod[fn]) == "function" then rawset(mod, fn, retTrue) end
                local impl = rawget(mod, "__inner_impl")
                if type(impl) == "table" and type(impl[fn]) == "function" then rawset(impl, fn, retTrue) end
            end
        end
    end
    ...
    if type(CDataTable) == "table" and type(CDataTable.GetTableData) == "function" then
        ...
        rawset(CDataTable, "GetTableData", function(tbl, id, ...)
            ...
            if type(id) == "number" and id >= 1000 then
                return { ID = id, Path = "", Name = "X3Unlock", IsValid = true, CanUse = true, IsOwn = true, IsUnlock = true, Level = 7, MaxLevel = 7 }
            end
            return nil
        end)
    end
    ...
    if type(PufferMgr) == "table" then
        if type(PufferMgr.GetState) == "function" then rawset(PufferMgr, "GetState", function(...) return 2 end) end
        if type(PufferMgr.IsDownloaded) == "function" then rawset(PufferMgr, "IsDownloaded", retTrue) end
    end
end
...
_G.X3ApplyOutfit = function(Character)
    ...
    local suitVal = _G.HK_Settings.SkinSuit or 1
    local suitID = 1407961
    if suitVal > 1 then
        local suitTable = {
            1407961, 1408801, 1408802, 1408803, 1408804, 1408805, 1408806, 1408807, 1408808, 1408809,
            1408810, 1408811, 1408812, 1408813, 1408814, 1408815, 1408816, 1408817, 1408818, 1408819,
            1408820, 1408821, 1408822, 1408823, 1408824, 1408825, 1408826, 1408827, 1408828, 1408829,
            1408830, 1408831, 1408832, 1408833, 1408834, 1408835, 1408836, 1408837, 1408838, 1408839,
            1408840, 1408841, 1408842, 1408843, 1408844, 1408845, 1408846, 1408847, 1408848, 1408849
        }
        suitID = suitTable[suitVal] or (1400000 + suitVal)
    end

    local bagVal = _G.HK_Settings.SkinBag or 1
    local bagID = 501003
    if bagVal > 1 then
        local bagTable = {
            501003, 501004, 501005, 501006, 501007, 501008, 501009, 501010, 501011, 501012, 501013, 501014, 501015
        }
        bagID = bagTable[bagVal] or (501000 + bagVal)
    end

    local helmVal = _G.HK_Settings.SkinHelmet or 1
    local helmID = 502003
    if helmVal > 1 then
        local helmTable = {
            502003, 502004, 502005, 502006, 502007, 502008, 502009, 502010, 502011, 502012, 502013, 502014, 502015
        }
        helmID = helmTable[helmVal] or (502000 + helmVal)
    end
...
local function GetCustomSkinID(baseID)
    if not baseID or baseID <= 0 then return 0 end
    local skinVal = 1
    local name = ""
    if baseID == 101004 then
        skinVal = _G.HK_Settings.SkinM416 or 1
        name = "M416"
    elseif baseID == 101001 then
        skinVal = _G.HK_Settings.SkinAKM or 1
        name = "AKM"
    elseif baseID == 101003 then
        skinVal = _G.HK_Settings.SkinSCAR or 1
        name = "SCAR"
    elseif baseID == 101008 then
        skinVal = _G.HK_Settings.SkinM762 or 1
        name = "M762"
    elseif baseID == 101006 then
        skinVal = _G.HK_Settings.SkinAUG or 1
        name = "AUG"
    elseif baseID == 103003 then
        skinVal = _G.HK_Settings.SkinUMP or 1
        name = "UMP"
    elseif baseID == 103001 then
        skinVal = _G.HK_Settings.SkinUZI or 1
        name = "UZI"
    elseif baseID == 101005 then
        skinVal = _G.HK_Settings.SkinGroza or 1
        name = "Groza"
    elseif baseID == 102005 then
        skinVal = _G.HK_Settings.SkinDBS or 1
        name = "DBS"
    elseif baseID == 102004 then
        skinVal = _G.HK_Settings.SkinS12K or 1
        name = "S12K"
    end
    
    local skinTables = {
        M416 = { 1101004007, 1101004001, 1101004002, 1101004003, 1101004004, 1101004005, 1101004006, 1101004008, 1101004009, 1101004010, 1101004011, 1101004012, 1101004013, 1101004014, 1101004015 },
        AKM = { 1101001007, 1101001001, 1101001002, 1101001003, 1101001004, 1101001005, 1101001006, 1101001008, 1101001009, 1101001010, 1101001011, 1101001012, 1101001013, 1101001014, 1101001015 },
        SCAR = { 1101003007, 1101003001, 1101003002, 1101003003, 1101003004, 1101003005, 1101003006, 1101003008, 1101003009, 1101003010, 1101003011, 1101003012, 1101003013, 1101003014, 1101003015 },
        M762 = { 1101008007, 1101008001, 1101008002, 1101008003, 1101008004, 1101008005, 1101008006, 1101008008, 1101008009, 1101008010, 1101008011, 1101008012, 1101008013, 1101008014, 1101008015 },
        AUG = { 1101006007, 1101006001, 1101006002, 1101006003, 1101006004, 1101006005, 1101006006, 1101006008, 1101006009, 1101006010, 1101006011, 1101006012, 1101006013, 1101006014, 1101006015 },
        UMP = { 1103003007, 1103003001, 1103003002, 1103003003, 1103003004, 1103003005, 1103003006, 1103003008, 1103003009, 1103003010, 1103003011, 1103003012, 1103003013, 1103003014, 1103003015 },
        UZI = { 1103001007, 1103001001, 1103001002, 1103001003, 1103001004, 1103001005, 1103001006, 1103001008, 1103001009, 1103001010, 1103001011, 1103001012, 1103001013, 1103001014, 1103001015 },
        Groza = { 1101005007, 1101005001, 1101005002, 1101005003, 1101005004, 1101005005, 1101005006, 1101005008, 1101005009, 1101005010, 1101005011, 1101005012, 1101005013, 1101005014, 1101005015 },
        DBS = { 1102005007, 1102005001, 1102005002, 1102005003, 1102005004, 1102005005, 1102005006, 1102005008, 1102005009, 1102005010, 1102005011, 1102005012, 1102005013, 1102005014, 1102005015 },
        S12K = { 1102004007, 1102004001, 1102004002, 1102004003, 1102004004, 1102004005, 1102004006, 1102004008, 1102004009, 1102004010, 1102004011, 1102004012, 1102004013, 1102004014, 1102004015 },
    }
    
    if name ~= "" and skinTables[name] then
        return skinTables[name][skinVal] or skinTables[name][1]
    end
...
_G.X3ApplyWeaponSkin = function(Character)
    ...
    for slot = 1, 3 do
        ...
        if slua.isValid(Weapon) and slua.isValid(Weapon.synData) then
            ...
            local skinID = GetCustomSkinID(baseID)
            if skinID <= 0 then goto continue end
            
            if Weapon.__x3_last_skin == skinID then goto continue end
            Weapon.__x3_last_skin = skinID
...
local function GetCustomVehicleSkinID(baseId)
    if not baseId or baseId <= 0 then return 0 end
    local skinVal = 1
    local name = ""
    local s = tostring(baseId)
    if s:find("1101") then
        skinVal = _G.HK_Settings.SkinDacia or 1
        name = "Dacia"
    elseif s:find("1102") or s:find("1108") then
        skinVal = _G.HK_Settings.SkinUAZ or 1
        name = "UAZ"
    elseif s:find("1105") then
        skinVal = _G.HK_Settings.SkinCoupe or 1
        name = "Coupe"
    elseif s:find("1103") then
        skinVal = _G.HK_Settings.SkinBuggy or 1
        name = "Buggy"
    elseif s:find("1104") then
        skinVal = _G.HK_Settings.SkinMirado or 1
        name = "Mirado"
    end
    return baseId + (skinVal * 1000)
end
...
_G.X3ApplyVehicleSkin = function(Character)
    ...
    local skinID = GetCustomVehicleSkinID(baseId)
